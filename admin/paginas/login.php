<?php
  ...
  if ( $_POST ) {
    ...
    if ( empty ( $login ) ) $msg = '<p class="alert alert-danger">Preencha o campo Login</p>';
    else if ( empty ( $senha ) ) $msg = '<p class="alert alert-danger">Preencha o campo Senha</p>';
    else {
      $sql = 
      "SELECT 
        id, nome, login, senha, foto 
      FROM 
        usuario 
      WHERE 
        login = ? 
      ...
      if ( empty ( $dados->id ) )   $msg = '<p class="alert alert-danger">O usuário não existe!</p>';
      else if ( !password_verify($senha, $dados->senha) )  $msg = '<p class="alert alert-danger">Senha incorreta</p>';
      else {
